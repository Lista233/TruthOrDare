...
{  
	...
	audio.src=(url)
	console.log(1, " at pages/tool/timer.uvue:39")
	//audio.autoplay=true;
	audio.play()
	audio.onEnded(()=>
	{
...
function start()
{
	...
	if(firsttouch)
	{
		...
		console.log(3, " at pages/tool/timer.uvue:57")
	...
	{
			stop()
			console.log(1, " at pages/tool/timer.uvue:73")
			 if (tid != -1) {
			    clearTimeout(tid) 
			}
...
function changeTime()
{
	let op:ShowModalOptions={title:'修改时间（单位：秒）',
							 ...
							 success:(res)=>{
								 if(res.cancel)
								 {return}
								 console.log('我成功了', " at pages/tool/timer.uvue:101");
